...
// Name-matching key: strips leading titles, lowercases, and removes ALL whitespace.
// Two rows sharing this key are the same person — if they point to DIFFERENT psychologist
// profiles, that's a duplicate that needs manual merging.
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-200/80">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3 space-y-1">
            <div className="text-[10px] uppercase tracking-wider font-bold text-blue-600/80">Therapist</div>
...
export default function TherapistsPage() {
  ...
  const load = async ({ silent = true } = {}) => {
    try {
      ...
      const r = await adminApi.getTherapistsSummary(dateParams);
      if (!r?.success) throw new Error(r?.error || 'Failed to load therapists');
      setRows(r?.data?.therapists || []);
      setStatsMeta(r?.data?.summary ?? null);
      if (!silent) showSuccess('Therapists refreshed');
    } catch (e) {
      showError(e?.message || 'Failed to load therapists', 'Load error');
    } finally {
      setLoading(false);
    }
  };
  ...
  // search/date filters) by the same name-matching key used to link
    ...
    <div className="p-4 md:p-8 max-w-[1280px] mx-auto space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-xl font-semibold text-gray-900">Therapists</div>
            <p className="text-xs text-slate-500 mt-0.5 max-w-xl">
              Every therapist on the platform, with their booking activity in the selected range.
            </p>
          </div>
        </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-amber-700 mt-0.5">
                ...
                Bookings could be splitting across them — one may be missing calendar sync, email, or phone.
