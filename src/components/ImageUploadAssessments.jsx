...
export default function ImageUploadAssessments({ currentImageUrl, onImageUpload, imageType, slug, label = "Upload Image" }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {previewUrl ? (
        <div className="relative">
          <div className="relative w-full h-48 border border-gray-300 rounded-lg overflow-hidden bg-gray-50">
            <Image src={normalizeImageUrl(previewUrl)} alt="Preview" fill className="object-contain" />
          </div>
          <div className="mt-2 flex gap-2">
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="flex-1 px-3 py-2 text-xs md:text-sm bg-blue-500 text-white rounded-md hover:bg-blue-600 disabled:opacity-50">{uploading ? 'Uploading...' : 'Change Image'}</button>
            <button type="button" onClick={handleRemove} disabled={uploading} className="px-3 py-2 text-xs md:text-sm bg-red-500 text-white rounded-md hover:bg-red-600 disabled:opacity-50">Remove</button>
          </div>
        </div>
      ) : (
        <div className="relative">
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="w-full h-32 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center hover:border-blue-500 hover:bg-blue-50 transition-colors disabled:opacity-50">
            {uploading ? (
      ...
      {currentImageUrl && (
        <div className="mt-2"><p className="text-xs text-gray-500 truncate" title={currentImageUrl}>Current URL: {currentImageUrl}</p></div>
      )}
    </div>
  );
}
